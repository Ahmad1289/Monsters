...
sudo_users = {sudo_add,bot_id,742682052,626400707}
...
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_monsters) do
...
local hash = database:sismember('monsters:'..bot_id..'creator:'..chat, user) 
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('monsters:'..bot_id..'owners:'..chat, user) 
if hash then 
return true 
else 
return false 
...
local hash = database:sismember('monsters:'..bot_id..'SET:BAN'..msg.chat_id_,msg.sender_user_id_) 
if hash then    
return true 
else 
return false 
...
{
{text = ''..(channel_ts or "TshAkE TEAM")..'', url=''..(channel_user_ts or 't.me/mons_bot')..''},
},
}
...
info = '👤┇العضو ~⪼ ['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'mons_bot')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
local list = redis:smembers('monsters:'..bot_id..'sudo:kekore')
for k,v in pairs(list) do
redis:del('monsters:'..bot_id.."sudo:add:reply1:gif"..v)   
redis:del('monsters:'..bot_id.."sudo:add:reply1:vico"..v)   
redis:del('monsters:'..bot_id.."sudo:add:reply1:stekr"..v)     
redis:del('monsters:'..bot_id.."sudo:add:reply:rd"..v)   
redis:del('monsters:'..bot_id.."sudo:addreply1:photo:gp"..v)
redis:del('monsters:'..bot_id.."sudo:addreply1:video:gp"..v)
redis:del('monsters:'..bot_id.."sudo:addreply1:document:gp"..v)
redis:del('monsters:'..bot_id.."sudo:addreply1:audio:gp"..v)
redis:del('monsters:'..bot_id..'sudo:kekore')
end
...
for k,v in pairs(list) do
if redis:get('monsters:'..bot_id.."sudo:add:reply1:gif"..v) then
db = 'متحركه 🎭'
elseif redis:get('monsters:'..bot_id.."sudo:add:reply1:vico"..v) then
db = 'بصمه 📢'
elseif redis:get('monsters:'..bot_id.."sudo:add:reply1:stekr"..v) then
db = 'ملصق 🃏'
elseif redis:get('monsters:'..bot_id.."sudo:add:reply:rd"..v) then
db = 'رساله ✉'
elseif redis:get('monsters:'..bot_id.."sudo:addreply1:photo:gp"..v) then
db = 'صوره 🎇'
elseif redis:get('monsters:'..bot_id.."sudo:addreply1:video:gp"..v) then
db = 'فيديو 📹'
elseif redis:get('monsters:'..bot_id.."sudo:addreply1:document:gp"..v) then
db = 'ملف 📁'
elseif redis:get('monsters:'..bot_id.."sudo:addreply1:audio:gp"..v) then
db = 'اغنيه 🎵'
end
...
local keko1 = redis:get('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
local test = redis:get('monsters:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('monsters:'..bot_id.."sudo:add:reply1:stekr"..test, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('monsters:'..bot_id.."sudo:add:reply1:vico"..test, msg.content_.voice_.voice_.persistent_id_)  
redis:set('monsters:'..bot_id.."sudo:addreply1:voice:caption:gp"..test,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('monsters:'..bot_id.."sudo:add:reply1:gif"..test, msg.content_.animation_.animation_.persistent_id_)  
redis:set('monsters:'..bot_id.."sudo:addreply1:animation:caption:gp"..test,(msg.content_.caption_ or ''))  
end  
if text then   
redis:set('monsters:'..bot_id.."sudo:add:reply:rd"..test, text)  
end  
if msg.content_.audio_ then
redis:set('monsters:'..bot_id.."sudo:addreply1:audio:gp"..test, msg.content_.audio_.audio_.persistent_id_)  
redis:set('monsters:'..bot_id.."sudo:addreply1:audio:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('monsters:'..bot_id.."sudo:addreply1:document:gp"..test, msg.content_.document_.document_.persistent_id_)  
redis:set('monsters:'..bot_id.."sudo:addreply1:document:caption:gp"..test,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('monsters:'..bot_id.."sudo:addreply1:video:gp"..test, msg.content_.video_.video_.persistent_id_)  
redis:set('monsters:'..bot_id.."sudo:addreply1:video:caption:gp"..test,(msg.content_.caption_ or ''))  
end
...
redis:set('monsters:'..bot_id.."sudo:addreply1:photo:gp"..test, photo_in_group)  
redis:set('monsters:'..bot_id.."sudo:addreply1:photo:caption:gp"..test,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
redis:set('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('monsters:'..bot_id..'sudo:msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:del('monsters:'..bot_id.."sudo:add:reply1:gif"..text)   
redis:del('monsters:'..bot_id.."sudo:add:reply1:vico"..text)   
redis:del('monsters:'..bot_id.."sudo:add:reply1:stekr"..text)     
redis:del('monsters:'..bot_id.."sudo:add:reply:rd"..text)   
redis:del('monsters:'..bot_id.."sudo:addreply1:photo:gp"..text)
redis:del('monsters:'..bot_id.."sudo:addreply1:video:gp"..text)
redis:del('monsters:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('monsters:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:sadd('monsters:'..bot_id..'sudo:kekore', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('monsters:'..bot_id.."sudo:addreply1:document:gp"..text)
redis:del('monsters:'..bot_id.."sudo:addreply1:audio:gp"..text)
redis:del('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:srem('monsters:'..bot_id..'sudo:kekore', text)
return false
end
end
if text == 'اضف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ', 1, 'md')
redis:set('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false 
end
if text == 'حذف رد للكل' and is_devabas(msg) then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ', 1, 'md')
redis:set('monsters:'..bot_id..'sudo:keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false 
end
if text and not database:get('monsters:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local anemi = redis:get('monsters:'..bot_id.."sudo:add:reply1:gif"..text)   
local veico = redis:get('monsters:'..bot_id.."sudo:add:reply1:vico"..text)   
local stekr = redis:get('monsters:'..bot_id.."sudo:add:reply1:stekr"..text)     
local text1 = redis:get('monsters:'..bot_id.."sudo:add:reply:rd"..text)   
local photo = redis:get('monsters:'..bot_id.."sudo:addreply1:photo:gp"..text)
local video = redis:get('monsters:'..bot_id.."sudo:addreply1:video:gp"..text)
local document = redis:get('monsters:'..bot_id.."sudo:addreply1:document:gp"..text)
local audio = redis:get('monsters:'..bot_id.."sudo:addreply1:audio:gp"..text)
------------------------------------------------------------------------
local video_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:video:caption:gp"..text) or '' )
local photo_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:photo:caption:gp"..text) or '' )
local document_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:document:caption:gp"..text) or '' )
local audio_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:audio:caption:gp"..text) or '' )
local animation_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:animation:caption:gp"..text) or '' )
local voice_caption = (redis:get('monsters:'..bot_id.."sudo:addreply1:voice:caption:gp"..text) or '' )
------------------------------------------------------------------------
if text1 then 
...
getUser(msg.sender_user_id_,monsters_re)
end
if stekr then 
...
for k,v in pairs(list) do
redis:del('monsters:'..bot_id.."add:reply1:gif"..v..msg.chat_id_)   
redis:del('monsters:'..bot_id.."add:reply1:vico"..v..msg.chat_id_)   
redis:del('monsters:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('monsters:'..bot_id.."add:reply:rd"..v..msg.chat_id_)   
redis:del('monsters:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_)
redis:del('monsters:'..bot_id..'kekore'..msg.chat_id_..'')
end
...
for k,v in pairs(list) do
if redis:get('monsters:'..bot_id.."add:reply1:gif"..v..msg.chat_id_) then
db = 'متحركه 🎭'
elseif redis:get('monsters:'..bot_id.."add:reply1:vico"..v..msg.chat_id_) then
db = 'بصمه 📢'
elseif redis:get('monsters:'..bot_id.."add:reply1:stekr"..v..msg.chat_id_) then
db = 'ملصق 🃏'
elseif redis:get('monsters:'..bot_id.."add:reply:rd"..v..msg.chat_id_) then
db = 'رساله ✉'
elseif redis:get('monsters:'..bot_id.."addreply1:photo:gp"..v..msg.chat_id_) then
db = 'صوره 🎇'
elseif redis:get('monsters:'..bot_id.."addreply1:video:gp"..v..msg.chat_id_) then
db = 'فيديو 📹'
elseif redis:get('monsters:'..bot_id.."addreply1:document:gp"..v..msg.chat_id_) then
db = 'ملف 📁'
elseif redis:get('monsters:'..bot_id.."addreply1:audio:gp"..v..msg.chat_id_) then
db = 'اغنيه 🎵'
end
text = text.."<b>|"..k.."|</b>~⪼("..v..") » {"..db.."}\n"
if #text > 7000 then
...
local test = redis:get('monsters:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الرد', 1, 'md')
redis:set('monsters:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
if msg.content_.sticker_ then   
redis:set('monsters:'..bot_id.."add:reply1:stekr"..test..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_)  
end   
if msg.content_.voice_ then  
redis:set('monsters:'..bot_id.."add:reply1:vico"..test..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_)  
redis:set('monsters:'..bot_id.."addreply1:voice:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end   
if msg.content_.animation_ then   
redis:set('monsters:'..bot_id.."add:reply1:gif"..test..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_)  
redis:set('monsters:'..bot_id.."addreply1:animation:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end  
if text then   
redis:set('monsters:'..bot_id.."add:reply:rd"..test..msg.chat_id_, text)  
end  
if msg.content_.audio_ then
redis:set('monsters:'..bot_id.."addreply1:audio:gp"..test..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_)  
redis:set('monsters:'..bot_id.."addreply1:audio:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.document_ then
redis:set('monsters:'..bot_id.."addreply1:document:gp"..test..msg.chat_id_, msg.content_.document_.document_.persistent_id_)  
redis:set('monsters:'..bot_id.."addreply1:document:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.video_ then
redis:set('monsters:'..bot_id.."addreply1:video:gp"..test..msg.chat_id_, msg.content_.video_.video_.persistent_id_)  
redis:set('monsters:'..bot_id.."addreply1:video:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo_in_group = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
redis:set('monsters:'..bot_id.."addreply1:photo:gp"..test..msg.chat_id_, photo_in_group)  
redis:set('monsters:'..bot_id.."addreply1:photo:caption:gp"..test..msg.chat_id_,(msg.content_.caption_ or ''))  
end
return false  
end  
end
...
redis:del('monsters:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
redis:del('monsters:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
redis:del('monsters:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
redis:del('monsters:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
redis:del('monsters:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:sadd('monsters:'..bot_id..'kekore'..msg.chat_id_..'', text)
return false end
end
if text and text:match("^(.*)$") then
local keko1 = redis:get('monsters:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
redis:del('monsters:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
redis:del('monsters:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
redis:del('monsters:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
local anemi = redis:get('monsters:'..bot_id.."add:reply1:gif"..text..msg.chat_id_)   
local veico = redis:get('monsters:'..bot_id.."add:reply1:vico"..text..msg.chat_id_)   
local stekr = redis:get('monsters:'..bot_id.."add:reply1:stekr"..text..msg.chat_id_)     
local text1 = redis:get('monsters:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
local photo = redis:get('monsters:'..bot_id.."addreply1:photo:gp"..text..msg.chat_id_)
local video = redis:get('monsters:'..bot_id.."addreply1:video:gp"..text..msg.chat_id_)
local document = redis:get('monsters:'..bot_id.."addreply1:document:gp"..text..msg.chat_id_)
local audio = redis:get('monsters:'..bot_id.."addreply1:audio:gp"..text..msg.chat_id_)
------------------------------------------------------------------------
local video_caption = (redis:get('monsters:'..bot_id.."addreply1:video:caption:gp"..text..msg.chat_id_) or '' )
local photo_caption = (redis:get('monsters:'..bot_id.."addreply1:photo:caption:gp"..text..msg.chat_id_) or '' )
local document_caption = (redis:get('monsters:'..bot_id.."addreply1:document:caption:gp"..text..msg.chat_id_) or '' )
local audio_caption = (redis:get('monsters:'..bot_id.."addreply1:audio:caption:gp"..text..msg.chat_id_) or '' )
local animation_caption = (redis:get('monsters:'..bot_id.."addreply1:animation:caption:gp"..text..msg.chat_id_) or '' )
...
------------------------------------------------------------------------
if database:get('monsters:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and is_sudo(msg) then 
...
------------------------------------------------------------------------
if database:get("monsters:set_if_bc_new:"..bot_id..msg.sender_user_id_) and is_sudo(msg) then  
...
------------------------------------------------------------------------
if text and database:get("monsters:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
...
------------------------------------------------------------------------
local keko2 = redis:get('monsters:'..bot_id..'hres1'..msg.sender_user_id_..'')
if keko2 == 'msg' then
if text:match("^(.*)$") then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('monsters:'..bot_id..'hres1'..msg.sender_user_id_..'', 'no')
redis:set('monsters:'..bot_id..'hres', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false 
end
end
...
if database:get("lock_kansers:monsters"..msg.chat_id_..bot_id) then
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
local last_ = data.last_name_ or ''
local first_ = data.first_name_ or ''
taha = (first_..''..last_)
Num = (database:get('monsters:'..bot_id..'Num:kansers'..msg.chat_id_) or 25)
...
local taha = database:get('monsters:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)   
if taha then    
...
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
end
------------------------------------------------------------------------
...
------------------------------------------------------------------------
if database:get("lock_chat.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_fwd.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_stecker.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_note.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_photo.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_audeo.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_voice.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_video.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_gif.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_contect.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_files.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
...
if database:get("lock_mark.note:monsters"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
HTTPS.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false") 
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, msg.sender_user_id_)
end
...
localmonsters_wr = (database:get("monsters:not:word:"..bot_id..msg.chat_id_) or 100)
...
if database:get("lock_bot:monsters"..msg.chat_id_..bot_id) and not bot_id then
...
local monsterse = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. monsterse .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_monsters/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if (text == 'الملفات' ) then
local files_monsters = database:smembers("files"..bot_id)
local files = io.popen('cd files_monsters && ls'):read("*all")
local files_monsters2 = ''
for i=1,#files_monsters do
files_monsters2 = files_monsters2..'{'..files_monsters[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..files..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_monsters2..'', 1, 'html')
end
...
if (f:match('^(.*)(-frmonsters.lua)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_monsters/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(monstersTEAM/Tshakex)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على الفرخ رضا اللوكي + كيكو دروح اتلوك بعد 👻😂 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_monsters/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -frmonsters.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملفmonsters.lua لتبعبص ابني "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_monsters/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
...
os.execute('cd libs;wget https://raw.githubusercontent.com/Ahmad1289/Monsters/master/libs/utils.lua')
os.execute('wget https://raw.githubusercontent.com/Ahmad1289/Monsters/master/monsters.lua')
dofile('monsters.lua')  
return false
end
...
f:close()
local monsterse = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. monsterse .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'monsters.txt' .. '"'
io.popen(curl)
end
------------------------------------------------------------------------
...
redis:del('monsters:'..bot_id..'hres')
...
redis:set('monsters:'..bot_id..'hres1'..msg.sender_user_id_..'', 'msg')
return false 
end
...
local tsX_o = database:get("lock_edit:monsters:kick"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل بالطرد")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل بالطرد")
database:set("lock_edit:monsters:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:monsters:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
database:set("monsters:mute:deleta:msg:"..bot_id..msg.chat_id_,"monsters")
...
database:set("monsters:fel:o:me:"..bot_id..msg.chat_id_,a[2])
...
database:set("monsters_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok")
return "monsters"
end  
if (text and database:get("monsters_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف صلاحيه بنجاح', 1, 'md')
database:del("monsters:all_if:"..text..bot_id..msg.chat_id_)
database:del("monsters_del_if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end 
...
database:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,monsters_edis[1])
...
database:set("monsters:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("monsters:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = database:get("monsters:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
database:del("monsters:edit:text:su:new2:"..bot_id..msg.chat_id_..klma)
...
database:set('monsters:'..bot_id..'Num:kansers'..msg.chat_id_,Num)
send(msg.chat_id_, msg.id_, 1, '\n🔖┇تم وضع عدد حروف الاسم {'..Num..'} حرف',1, 'md')
end
if (text == "قفل الكانسر") then
local tsX_o = database:get("lock_kansers:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكانسر ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكانسر ")
database:set("lock_kansers:monsters"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_kansers:monsters"..msg.chat_id_..bot_id,"ok")
end
end
...
for k,v in pairs(list) do  
database:del('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('monsters:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)  
...
for k,v in pairs(list) do  
local taha = database:get('monsters:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
database:set('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
database:set('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set('monsters:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd('monsters:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
database:del('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get('monsters:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set('monsters:'..bot_id.."filtr1:add:reply:rd"..test..msg.chat_id_, text)  
database:hset('monsters:'..bot_id..'filters:'..msg.chat_id_, text, 'filtered')
end  
database:del('monsters:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
...
database:set('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del('monsters:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('monsters:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)  
...
local hash = 'monsters:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash)
text = "👥┇قائمة المقيدين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('monsters'..bot_id.."user:"..v)
...
local hash = 'monsters:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash) 
for k,v in pairs(list) do database:del('monsters:'..bot_id..'res:'..msg.chat_id_) 
...
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(res[2] or 'monstersteam')..')\n☑️┇تم تقييده'
end
else
texts = '✖️┇خطاء'
end
...
database:sadd('monsters:'..bot_id..'res:'..msg.chat_id_, apba[2])
...
local tsX_o = database:get("lock_tag.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التاك بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التاك بالتقييد")
database:set("lock_tag.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الشارحه بالتقيد") then
local tsX_o = database:get("lock_sarha.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الشارحه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الشارحه بالتقييد")
database:set("lock_sarha.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المتحركه بالتقيد") then
local tsX_o = database:get("lock_gif.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل المتحركه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المتحركه بالتقييد")
database:set("lock_gif.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملفات بالتقيد") then
local tsX_o = database:get("lock_files.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملفات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملفات بالتقييد")
database:set("lock_files.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الماركدون بالتقيد") then
local tsX_o = database:get("lock_mark.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الماكدون بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الماركدون بالتقييد")
database:set("lock_mark.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصور بالتقيد") then
local tsX_o = database:get("lock_photo.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصور بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصور بالتقييد")
database:set("lock_photo.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملصقات بالتقيد") then
local tsX_o = database:get("lock_stecker.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملصقات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملصقات بالتقييد")
database:set("lock_stecker.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الفيديو بالتقيد") then
local tsX_o = database:get("lock_video.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الفيديو بالتقييد")
database:set("lock_video.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانلاين بالتقيد") then
local tsX_o = database:get("lock_inline.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانلاين بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانلاين بالتقييد")
database:set("lock_inline.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الدردشه بالتقيد") then
local tsX_o = database:get("lock_chat.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الدردشه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الدردشه بالتقييد")
database:set("lock_chat.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل التوجيه بالتقيد") then
local tsX_o = database:get("lock_fwd.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التوجيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التوجيه بالتقييد")
database:set("lock_fwd.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الاغاني بالتقيد") then
local tsX_o = database:get("lock_audeo.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الاغاني بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الاغاني بالتقييد")
database:set("lock_audeo.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصوت بالتقيد") then
local tsX_o = database:get("lock_voice.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصوت بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصوت بالتقييد")
database:set("lock_voice.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الجهات بالتقيد") then
local tsX_o = database:get("lock_contact.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الجهات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الجهات بالتقييد")
database:set("lock_contact.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل العربيه بالتقيد") then
local tsX_o = database:get("lock_ar.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل العربيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل العربيه بالتقييد")
database:set("lock_ar.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانكليزيه بالتقيد") then
local tsX_o = database:get("lock_en.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانكليزيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانكليزيه بالتقييد")
database:set("lock_en.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الروابط بالتقيد") then
local tsX_o = database:get("lock_link.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الروابط بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الروابط بالتقييد")
database:set("lock_link.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المعرف بالتقيد") then
local tsX_o = database:get("lock_username.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
 tsX000("lock",msg,"☑️┇بالفعل تم قفل المعرف بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المعرف بالتقييد")
database:set("lock_username.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل بصمه الفيديو بالتقيد") then
local tsX_o = database:get("lock_note.note:monsters"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل بصمه الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل بصمه الفيديو بالتقييد")
database:set("lock_note.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_tag.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_sarha.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_gif.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_files.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_mark.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_photo.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_stecker.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_video.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_inline.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_chat.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_fwd.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_audeo.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_voice.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_contact.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_ar.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_en.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_username.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_note.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_link.note:monsters"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
if database:get("lock_audeo.note:monsters"..msg.chat_id_..bot_id) then
 ...
 ------------
if database:get("lock_inline.note:monsters"..msg.chat_id_..bot_id) then
 mute_in = '✔┇'
else
 mute_in = '✖┇'
 end
...
lock_tag = '✖┇'
 ...
 ------------
if database:get("lock_contact.note:monsters"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
 end
...
if redis:sismember('monsters:'..bot_id..'dev',result.id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('monsters:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('monsters:'..bot_id..'creator:'..msg.chat_id_, result.id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('monsters:'..bot_id..'owners:'..msg.chat_id_, result.id_) then
own = 'مدير ،' else own = '' end
if database:sismember('monsters:'..bot_id..'mods:'..msg.chat_id_, result.id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('monsters:'..bot_id..'vipgp:'..msg.chat_id_, result.id_) then
vip = 'مميز ،' else vip = ''
end
...
database:srem('monsters:'..bot_id..'dev', result.id_)
database:del('monsters:'..bot_id..'sudoo'..result.id_..'', 'no')
database:srem('monsters:'..bot_id..'creatorbasic:'..msg.chat_id_,result.id_)
database:srem('monsters:'..bot_id..'creator:'..msg.chat_id_, result.id_)
database:srem('monsters:'..bot_id..'owners:'..msg.chat_id_, result.id_)
database:srem('monsters:'..bot_id..'mods:'..msg.chat_id_, result.id_)
database:srem('monsters:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
if redis:sismember('monsters:'..bot_id..'dev',result.sender_user_id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('monsters:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('monsters:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('monsters:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_) then
own = 'مدير ،' else own = '' end
if database:sismember('monsters:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('monsters:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_) then
vip = 'مميز ،' else vip = ''
end
...
database:del('monsters:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
database:srem('monsters:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
...
------------------------------------------------------------------------
if (text and text == (redis:get('monsters:'..bot_id..'name_bot') or 'مونستر'))then
name_bot = (redis:get('monsters:'..bot_id..'name_bot') or 'مونستر')
local namebot = {
...
name = katu[math.random(#katu)]
database:del('monsters:'..bot_id..'l:idss'..msg.chat_id_)
...
database:del('monsters:'..bot_id..'l:idst'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == ''..(database:get('monsters:'..bot_id..':Set_alii:'..msg.chat_id_) or '57999')..'' then -- // المختلف
if not database:get('monsters:'..bot_id..'l:idss'..msg.chat_id_) then 
database:incrby('monsters:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('monsters:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
...
database:set('monsters:'..bot_id..'l:idss'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == ''..(database:get('monsters:'..bot_id..':Set_Amthlh:'..msg.chat_id_) or '6499')..'' then -- // امثله
if not database:get('monsters:'..bot_id..'l:idst'..msg.chat_id_) then 
database:incrby('monsters:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('monsters:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
...
database:set('monsters:'..bot_id..'l:idst'..msg.chat_id_,true)
end
------------------------------------------------------------------------
...
local text =  [[
اهلا بك في سورس مونسترس تريد تنصب السورز راسل المطورين 
🌐┇monsters 
🦁┇[قناه السورس](t.me/mons_bot)
[ممبرمج السورس احمد](http://t.me/A_cobra_A)
[ممبرمج السورس علي](t.me/barko_b)
[ممبرمج السورس امير](https://t.me/hmody12/648)
[تواصلنا](قريبا)
]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
database:hset('monsters:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
if database:get("lock_botAndBan:monsters"..msg.chat_id_..bot_id) and not bot_id then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
changeChatMemberStatus(msg.chat_id_, msg.sender_user_id_, "Kicked")
end
end
...
print(monsters_edit_text)
if monsters_edit_text then
data.message_.content_.text_ = (monsters_edit_text or data.message_.content_.text_)
end
end
end
------------------------------------------------------------------------
...
------------------------------------------------------------------------
monsters_run_(msg,data)
monsters_run_file(Data_monsters)
------------------------------------------------------------------------
--         »»                 Start UpdateChat                         ««              --
...
if database:get("lock_edit:monsters"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
...
elseif database:get("lock_edit:monsters:kick"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
...
database:srem("monsters:gog"..bot_id,v) 
...
database:sadd("monsters:gog"..bot_id,v) 
database:sadd('monsters:'..bot_id..'pro:groups',v) 
database:sadd( 'monsters:'..bot_id.."groups",v) 
...
( تخمط انيج اختك )
  monsters
-------------------------------
sudo > @A_cobra_A @barko_b @hmody12
--]]
